...
function displayHelp() {
	if (!helpOpen) {
		...
		if (popupPosition[0] > window.innerWidth - document.getElementById('downloadPopup').offsetWidth - document.getElementById('popupHelpDialog').offsetWidth - 30) {
			popupPosition[0] = window.innerWidth - document.getElementById('downloadPopup').offsetWidth - document.getElementById('popupHelpDialog').offsetWidth - 30;
			updatePosition();
		}
...
document.getElementById('downloadPopup').children[0].addEventListener('mousedown', function(e) {
	mouseDragging = true;
	mouseOffset = [popupPosition[0] - e.x, popupPosition[1] - e.y];
});
...
document.body.addEventListener('mousemove', function(e) {
	if (mouseDragging) {
		popupPosition = [e.x + mouseOffset[0], e.y + mouseOffset[1]];
		popupPosition[0] = Math.max(Math.min(popupPosition[0], window.innerWidth - document.getElementById('downloadPopup').offsetWidth), 0);
		popupPosition[1] = Math.max(Math.min(popupPosition[1], window.innerHeight - document.getElementById('downloadPopup').offsetHeight), 0);
		updatePosition();
	}
});
